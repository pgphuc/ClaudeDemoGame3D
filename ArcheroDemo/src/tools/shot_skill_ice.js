/* shot_skill_ice.js — [VÒNG D-5] chụp VFX skill "ice".
...
window.__runTest = function () {
  ...
  var cast = g.castSkill(1);
  for (var t = 0; t < 17; t++) { g.state.hero.hp = g.state.hero.hpMax; g.tick(1 / 60); }
  var fx = g._internal.skillFx();
  g.state.phase = 'paused';
  return { skill: 'ice', cast: cast, fxAlive: fx.fx, fireballs: fx.fireballs,
           clouds: fx.clouds, enemies: g.state.enemies.length,
           skillState: g.getSkillState()[1] };
};
