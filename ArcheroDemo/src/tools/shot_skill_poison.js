/* shot_skill_poison.js — [VÒNG D-5] chụp VFX skill "poison".
...
window.__runTest = function () {
  ...
  var cast = g.castSkill(3);
  for (var t = 0; t < 90; t++) { g.state.hero.hp = g.state.hero.hpMax; g.tick(1 / 60); }
  var fx = g._internal.skillFx();
  g.state.phase = 'paused';
  return { skill: 'poison', cast: cast, fxAlive: fx.fx, fireballs: fx.fireballs,
           clouds: fx.clouds, enemies: g.state.enemies.length,
           skillState: g.getSkillState()[3] };
};
